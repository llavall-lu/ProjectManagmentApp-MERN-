...
interface ChatMessage { // Create a type for chat messages
  sender: string; 
  message: string; 
}

const ChatPage = () => { // Create the ChatPage component
  ...
  const connectWebSocket = () => {
    ...
    ws.current.onopen = () => { // Function to handle WebSocket open event
    ...
    ws.current.onclose = () => { // Function to handle WebSocket close event
      console.log("WebSocket closed. Attempting to reconnect...");
      setTimeout(connectWebSocket, 3000); // Try to reconnect after 3 seconds
    };

    ws.current.onmessage = (e) => { // Function to handle WebSocket messages
      const processMessage = (messageData: string) => { // Function to process the message data
        try {
          ...
          if (parsedMessage.type === 'history') {
            ...
            const newMessage: ChatMessage = { sender: senderName, message: messageText }; // Create the message object
      ...
      if (e.data instanceof Blob) { // Check if the message is a Blob
        const reader = new FileReader(); // Create a FileReader
        reader.onload = function() { 
          if (typeof reader.result === 'string') { // Check if the result is a string
  ...
  useEffect(() => {
    if (user && (!ws.current || ws.current.readyState === WebSocket.CLOSED)) { // Check if the user is logged in and the WebSocket connection is closed
      connectWebSocket(); // Initialize WebSocket connection
    }
    return () => { 
      if (ws.current) { // Check if the WebSocket connection exists
        ws.current.close(); // Close the WebSocket connection
      }
    };
  }, [user]);

  const sendMessage = () => { // Function to send a message to the WebSocket server
    if (input.trim() !== '' && ws.current && ws.current.readyState === WebSocket.OPEN) { // Check if the input is not empty and the WebSocket connection is open
      const messageToSend = { name: user?.fullName || 'Anonymous', text: input }; // Create the message object
      ws.current.send(JSON.stringify(messageToSend)); // Send the message to the WebSocket server
  
      // Add the message to the chat history
      setMessages(prevMessages => [...prevMessages, { sender: messageToSend.name, message: messageToSend.text }]); 
      ...
      <ChatContainer>
        <ChatMessages>
          {messages.map((msg, index) => (
            <Message key={index}>{msg.sender}: {msg.message}</Message>
          ))}
